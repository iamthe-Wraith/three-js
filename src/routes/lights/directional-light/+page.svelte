<script lang="ts" type="module">
  ...
  /**
   * is very similar to the ambient light, except it supports direction.
   * 
   * think of the rays of light light like an Orthographic camera instead
   * of a Perspective camera. the rays of light run parallel to each other
   * rather than converging on a single point.
  */
  ...
  onMount(async () => {
    ...
    // this allows you to tell the light to point at a specific object
    light.target = torus[0];

    /**
     * you can also set the target manually, but then you need
     * to add the light target to the scene
     */
